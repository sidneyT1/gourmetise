<template>
  <v-container class="pa-4 fond-page" style="max-width: 700px;">
    ...
    <form @submit.prevent="submit">
      ...
      <v-checkbox v-model="checkbox" :rules="checkboxRules" label="En soumettant ce formulaire, J’ai lu et accepté les conditions d’utilisation relatives à la collecte de mes données." />

      <v-row class="validation" justify="center">
        <v-btn @click="goBack" class="large-button me-7">Retour</v-btn>
        <v-btn type="submit" class="large-button" :disabled="!isFormValid">Valider</v-btn> 
      </v-row>
    </form>
  </v-container>
</template>
